...
%Note that h(x)k = a(3) is the activation  of the k-th output unit.
di = -1 * ys .* log(a3) - (1 - ys) .* log(1 - a3);
J 	=  sum( sum(di')' ) / m;

% Regularized cost function
The1 = Theta1(:, 2:size(Theta1, 2));
The2 = Theta2(:, 2:size(Theta2, 2));
reg = (lambda / (2*m)) * (sum(sum((The1.^ 2)')') + sum(sum((The2.^ 2)')') ) ;
J = J + reg;


delta3  =  a3 - ys; % 5000 X 10
